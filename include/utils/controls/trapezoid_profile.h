...
/**
 * trapezoid_profile_segment_t is a description of one constant acceleration
 * segment of a trapezoid motion profile
 */
typedef struct {
 ...
 *
 * Using this information, a parametric function is generated, with a period of
 * acceleration, constant velocity, and deceleration. The velocity graph usually
 * looks like a trapezoid, giving it its name.
 *
 * If the maximum velocity is set high enough, this will become a S-curve
 * profile, with only acceleration and deceleration.
 *
 * If the initial velocity is in the wrong direction, the profile will first
 * come to a stop, then continue a normal trapezoid profile.
 *
 * If the initial velocity is higher than the maximum velocity, the profile will
 * first try to achieve the maximum velocity.
 *
 * If the end velocity is not achievable, the profile will try to get as close
 * as possible. The end velocity must be in the direction of the end point.
 *
 * This class is designed for use in properly modelling the motion of the robots
 * to create a feedfoward and target for PID. Acceleration and Maximum velocity
 * should be measured on the robot and tuned down slightly to account for
 * battery drop.
 *
...
public:
  ...
  /**
   * @brief Run the trapezoidal profile based on the time and distance that's
   * elapsed
   *
  ...
  /**
   * @brief set_accel sets the acceleration this profile will use (the left and
   * right legs of the trapezoid)
   *
  ...
  /**
   * @brief sets the maximum velocity for the profile (the height of the top of
   * the trapezoid)
   *
  ...
  /**
   * @brief uses the kinematic equations to and specified accel and max_v to
   * figure out how long moving along the profile would take
   *
...
private:
  ...
  /**
   * Calculate a trapezoid segment given a target velocity that accelerates to
   * the given velocity
   *
