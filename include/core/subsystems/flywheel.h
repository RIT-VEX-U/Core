 ...
 * It gives multiple options for what control system to use in order to control wheel velocity and
 * functions alerting the user when the flywheel is up to speed. Flywheel is a set and forget class.
 * Once you create it you can call spin_rpm or stop on it at any time and it will take all necessary
...
class Flywheel {
   public:
     ...
     * @param helper      a feedforward config (only kV is used) to help the feedback controller
     * along
     * @param ratio       ratio of the gears from the motor to the flywheel just multiplies the
     * velocity
     ...
     * FOR USE BY OPCONTROL AND AUTONOMOUS - this only applies if the target_rpm thread is not
     * running
    ...
    /**
     * Stops the motors. If manually spinning, this will do nothing just call spin_mainual(0.0) to
     * send 0 volts
     */
    void stop();
    ...
    /**
     * @brief Creates a new auto command that will hold until the flywheel has its target as defined
     * by its feedback controller
     * @return an auto command to add to a command controller
     */
   ...
   private:
    ...
    /**
     * make a measurement of the current target_rpm of the flywheel motor and return a smoothed
     * version
     */
    double measure_RPM();
