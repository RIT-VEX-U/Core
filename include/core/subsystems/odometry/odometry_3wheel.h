 ...
 * Odometry3Wheel
 *
 * This class handles the code for a standard 3-pod odometry setup, where there are 3 "pods" made up
 * of undriven (dead) wheels connected to encoders in the following configuration:
 *
 *  +Y   ---------------
 *  ^    |             |
 *  |    |             |
 ...
 *  +-------------------> + X
 *
 * Where O is the center of rotation. The robot will monitor the changes in rotation of these wheels
 * and calculate the robot's X, Y and rotation on the field.
 *
...
class Odometry3Wheel : public OdometryBase {
   public:
    /**
     * odometry3wheel_cfg_t holds all the specifications for how to calculate position with 3
     * encoders See the core wiki for what exactly each of these parameters measures
     */
    typedef struct {
        double wheelbase_dist; /**< distance from the center of the left wheel to the center of the
                                  right wheel*/
        double off_axis_center_dist; /**< distance from the center of the robot to the center off
                                        axis wheel*/
   ...
   private:
    /**
     * Calculation method for the robot's new position using the change in encoders, the old
     * position, and the robot's configuration. This uses a series of arclength formulae for finding
     * distance driven and change in angle. Then vector math is used to combine it with the robot's
     * old position data
     *
