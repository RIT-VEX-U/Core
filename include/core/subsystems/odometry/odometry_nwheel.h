   ...
   public:
    ...
    Pose2d update() override {
        ...
        if (update_vel_accel) {
            ...
            speed_local =
                    updated_pos.translation().distance(last_pos.translation()) / tmr.time(sec);
            ...
            ang_speed_local =
                    smallest_angle(
                            updated_pos.rotation().degrees(), last_pos.rotation().degrees()
                    ) /
                    tmr.time(sec);
    ...
    void set_position(const Pose2d& newpos) override {
        mut.lock();
        angle_offset =
                newpos.rotation().degrees() - (current_pos.rotation().degrees() - angle_offset);
        mut.unlock();
    ...
    Pose2d get_position(void) {
        ...
        Pose2d with_wrapped_angle(
                without_wrapped_angle.translation(),
