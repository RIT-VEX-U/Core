 ...
 * OdometryNWheel
 *
 * This class handles the code for an N-pod odometry setup, where there are N <WHEELS> free spinning
 * omni wheels (dead wheels) placed in any known configuration on the robot.
 *
 ...
 *  +-------------------> + X
 *
 * Where O is the center of rotation. The robot will monitor the changes in rotation of these
 * wheels, use this to calculate a pose delta, then integrate the deltas over time to determine the
 * robot's position.
 *
...
class OdometryNWheel : public OdometryBase {
   public:
     ...
     */
    OdometryNWheel(
            const std::array<CustomEncoder, WHEELS>& encoders,
            const std::array<tracking_wheel_cfg_t, WHEELS> wheel_configs, vex::inertial* imu,
            bool is_async
    )
        ...
        transfer_matrix_pseudoinverse =
                transfer_matrix.completeOrthogonalDecomposition().pseudoInverse();
        angle_offset = 0;
    ...
    Pose2d update() override {
        ...
        if (update_vel_accel) {
            ...
            speed_local = updated_pos.translation_.distance(last_pos.translation_).to(units::in) /
                          tmr.time(vex::sec);
            ...
            ang_speed_local =
                    smallest_angle(updated_pos.rotation_.degrees(), last_pos.rotation_.degrees()) /
                    tmr.time(vex::sec);
    ...
    void set_position(const Pose2d& newpos) override {
        mut.lock();
        angle_offset =
                newpos.rotation_.radians() - (current_pos.rotation_.radians() - angle_offset);
        mut.unlock();
    ...
    Pose2d get_position(void) {
        ...
        Pose2d with_wrapped_angle(
                without_wrapped_angle.translation_,
                without_wrapped_angle.rotation_.wrapped_radians_360()
        );
   ...
   private:
    /**
     * Calculation method for the robot's new position using the change in encoders, and the old
     * pose, the wheel configurations are stored as class members.
     *
    ...
    Pose2d calculate_new_pos(Eigen::Vector<double, WHEELS> radian_deltas, Pose2d old_pose) {
        // Mr T = E -> Mr^{+} E = T
        // We take the diagonal of radian_deltas to do a coefficient wise multiplication rather than
        // a dot product
        Eigen::Vector3d pose_delta =
                transfer_matrix_pseudoinverse * (radian_deltas.asDiagonal() * wheel_radii);
        Eigen::Vector3d old_pose_vector{
                old_pose.x().to(units::in), old_pose.y().to(units::in), old_pose.rotation_.radians()
        };
        ...
        Pose2d new_pose = old_pose_Pose2d.exp(Twist2d(
                units::Length(pose_delta[0], units::in), units::Length(pose_delta[1], units::in),
                units::Angle(pose_delta[2], units::rad)
        ));

        // simply replaces the calculated angle with the imu angle directly
        if (imu != nullptr) {
            new_pose = Pose2d(new_pose.translation_, angle);
        }
        return new_pose;
    }
