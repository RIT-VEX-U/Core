...
class TankDrive {
 public:
   ...
   * @param config the configuration specification defining physical dimensions about the robot. See
   * robot_specs_t for more info
  ...
  AutoCommand* DriveForwardCmd(
      Feedback& fb, double dist, vex::directionType dir = vex::forward, double max_speed = 1.0,
      double end_speed = 0.0
  );
  ...
  AutoCommand* PurePursuitCmd(
      PurePursuit::Path path, vex::directionType dir, double max_speed = 1, double end_speed = 0
  );
  AutoCommand* PurePursuitCmd(
      Feedback& feedback, PurePursuit::Path path, vex::directionType dir, double max_speed = 1,
      double end_speed = 0
  );
   ...
   *
   * @param degrees     degrees by which we will turn relative to the robot (+) turns ccw, (-) turns
   * cw
   * @param feedback    the feedback controller we will use to travel. controls the rate at which we
   ...
   */
  bool pure_pursuit(
      PurePursuit::Path path, vex::directionType dir, Feedback& feedback, double max_speed = 1,
      double end_speed = 0
  );
 ...
 private:
  ...
  bool func_initialized =
      false;  ///< used to control initialization of autonomous driving. (you only wan't to set the
