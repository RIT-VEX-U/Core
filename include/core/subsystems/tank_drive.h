 ...
 * A tank drive system, sometimes called differential drive, has a motor (or group of synchronized
 * motors) on the left and right side
 */
class TankDrive {
   public:
     ...
     * @param config the configuration specification defining physical dimensions about the robot.
     * See robot_specs_t for more info
     * @param odom an odometry system to track position and rotation. this is necessary to execute
     * autonomous paths
     */
    TankDrive(
    ...
    AutoCommand* DriveToPointCmd(
            Translation2d pt, vex::directionType dir = vex::forward, double max_speed = 1.0,
            double end_speed = 0.0
    );
    AutoCommand* DriveToPointCmd(
            Feedback& fb, Translation2d pt, vex::directionType dir = vex::forward,
            double max_speed = 1.0, double end_speed = 0.0
    );

    AutoCommand* DriveToPointCmd(
            double x, double y, vex::directionType dir = vex::forward, double max_speed = 1.0,
            double end_speed = 0.0
    );

    AutoCommand* DriveForwardCmd(
            double dist, vex::directionType dir = vex::forward, double max_speed = 1.0,
            double end_speed = 0.0
    );
    AutoCommand* DriveForwardCmd(
            Feedback& fb, double dist, vex::directionType dir = vex::forward,
            double max_speed = 1.0, double end_speed = 0.0
    );

    AutoCommand* TurnToHeadingCmd(double heading, double max_speed = 1.0, double end_speed = 0.0);
    AutoCommand* TurnToHeadingCmd(
            Feedback& fb, double heading, double max_speed = 1.0, double end_speed = 0.0
    );

    AutoCommand* TurnToPointCmd(
            double x, double y, vex::directionType dir = vex::directionType::fwd,
            double max_speed = 1.0, double end_speed = 0.0
    );
    AutoCommand* TurnToPointCmd(
            Translation2d point, vex::directionType dir = vex::directionType::fwd,
            double max_speed = 1.0, double end_speed = 0.0
    );

    AutoCommand* TurnDegreesCmd(double degrees, double max_speed = 1.0, double start_speed = 0.0);
    AutoCommand* TurnDegreesCmd(
     ...
     * @param max_speed  the maximum percentage of robot speed at which the robot will travel. 1 =
     * full power
     * @param end_speed    the movement profile will attempt to reach this velocity by its
     * completion
     * @return true when we have reached our target distance
     */
    bool drive_forward(
            double inches, vex::directionType dir, Feedback& feedback, double max_speed = 1,
            double end_speed = 0
    );
     ...
     */
    bool drive_forward(
            double inches, vex::directionType dir, double max_speed = 1, double end_speed = 0
    );
     ...
     *
     * @param degrees     degrees by which we will turn relative to the robot (+) turns ccw, (-)
     * turns cw
     * @param feedback    the feedback controller we will use to travel. controls the rate at which
     * we accelerate and drive.
     * @param max_speed   the maximum percentage of robot speed at which the robot will travel. 1 =
     * full power
     */
    bool turn_degrees(
            double degrees, Feedback& feedback, double max_speed = 1, double end_speed = 0
    );
     ...
     */
    bool drive_to_point(
            double x, double y, vex::directionType dir, Feedback& feedback, double max_speed = 1,
            double end_speed = 0
    );
     ...
     */
    bool drive_to_point(
            double x, double y, vex::directionType dir, double max_speed = 1, double end_speed = 0
    );
    ...
    bool turn_to_heading(
            double heading_deg, Feedback& feedback, double max_speed = 1, double end_speed = 0
    );
    /**
     * Turn the robot in place to an exact heading relative to the field.
   ...
   private:
    ...
    double arcade_throttle = 0.0;  ///< previous arcade throttle; reset by stop() (used for cheesy style arcade)
