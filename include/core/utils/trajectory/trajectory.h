   ...
   public:
    ...
    struct State {
        ...
        State interpolate(State end_value, double i) const {
            const units::Time new_t =
                    1_s * (t.internal() + (end_value.t.internal() - t.internal()) * i);
            ...
            const units::Length distance =
                    units::Length(end_value.pose.translation().distance(pose.translation()));
            const double interpolation_frac =
                    distance > 1E-9_in ? (new_s / distance).internal() : i;
            ...
            units::Curvature new_curvature =
                    1_radpm *
                    (curvature.internal() +
                     (end_value.curvature.internal() - curvature.internal()) * interpolation_frac);
    ...
    State sample(units::Time t) const {
        ...
        return prev_sample->interpolate(
                *sample, ((t - prev_sample->t) / (sample->t - prev_sample->t)).internal()
        );
    }
