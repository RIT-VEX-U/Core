...
class RectangularRegionConstraint : public TrajectoryConstraint {
   public:
    ...
    RectangularRegionConstraint(
            const Translation2d& bottom_left,
            const Translation2d& top_right,
            const TrajectoryConstraint& constraint
    )
        : bottom_left_(bottom_left), top_right_(top_right), constraint_(constraint.clone()) {}
    ...
    TrajectoryConstraint::MinMax min_max_acceleration(
            const Pose2d& pose, units::Curvature curvature, units::Velocity velocity
    ) const override {
        if (is_in_region(pose.translation())) {
    ...
    std::unique_ptr<TrajectoryConstraint> clone() const override {
        return std::make_unique<RectangularRegionConstraint>(
                bottom_left_, top_right_, *constraint_
        );
    }
