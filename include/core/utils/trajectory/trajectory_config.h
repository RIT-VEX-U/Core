...
class TrajectoryConfig {
   public:
     ...
     */
    TrajectoryConfig(
            units::Velocity max_velocity, units::Acceleration max_acceleration, bool reversed
    )
    ...
    /**
     * @brief Constructs TrajectoryConfig with max velocity, max acceleration, and track width
     * constraint.
     * @param max_velocity Max velocity limit.
     * @param max_acceleration Max acceleration limit.
    ...
    void add_event(const std::string &name, units::Length distance) {
        events_.push_back({distance, name});
    }
    ...
    void set_error_handler(std::function<void(const char *)> handler) {
    ...
    typename std::enable_if<
            std::is_base_of<TrajectoryConstraint, typename std::decay<Constraint>::type>::value,
            void>::type
    add_constraint(Constraint &&constraint) {
        typedef typename std::decay<Constraint>::type C;
        constraints_.emplace_back(
                std::unique_ptr<TrajectoryConstraint>(new C(std::forward<Constraint>(constraint)))
        );
    }
