...
#include "core/utils/controls/asymmetric_trapezoid_profile.h"
 ...
 * @date 7/13/2022
 */
class AsymmetricMotionController : public Feedback {
  public:
    /**
    ...
    typedef struct {
        ...
        double decel;                    ///< the most deceleration the robot can do
     ...
     */
    AsymmetricMotionController(a_m_profile_cfg_t &config);
    ...
    tune_feedforward(TankDrive &drive, OdometryBase &odometry, double pct = 0.6, double duration = 2);

  private:
    a_m_profile_cfg_t config;
