...
// Struct representing a state along the motion profile containing position, velocity and
// acceleration.
typedef struct {
...
/**
 * Class representing a trapezoidal motion profile. This consists of either two or three stages.
 * First, an acceleration stage where the velocity increases to a maximum. Then a cruise stage where
 * the velocity is constant, then a deceleration stage where the velocity decreases to zero.
 *
 * This implementation allows for different acceleration and deceleration rates.
 *
 * This is best used with LQR, as it tracks both velocity and position at the same time, however it
 * can also be used with PID and feedforward.
 *
