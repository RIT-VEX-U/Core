...
class SplinePath {
   public:
    ...
    static SplinePath from_waypoints(
            ...
            std::function<
                    std::unique_ptr<SplineBase>(const HermitePoint&, const HermitePoint&, double)>
                    factory,
            double max_err = 1e-4
    ) {
        SplinePath path;
     ...
     * @return SplineSample struct containing position, velocity, acceleration, heading, and
     * curvature.
     */
