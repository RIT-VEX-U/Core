 ...
 *
 * An exponential moving average is a way of smoothing out noisy data. For many sensor readings, the
 * noise is roughly symmetric around the actual value. This means that if you collect enough samples
 * those that are too high are cancelled out by the samples that are too low leaving the real value.
 *
 * A simple mobing average lags significantly with time as it has to counteract old samples. An
 * exponential moving average keeps more up to date by weighting newer readings higher than older
 * readings so it is more up to date while also still smoothed.
 *
 * The ExponentialMovingAverage class provides an simple interface to do this smoothing from our
 * noisy sensor values.
 *
 */
