...
constexpr Q from(double value, Q unit) {
...
using Exponentiated = Quantity<
    std::ratio_multiply<typename Q::mass, factor>,
...
using Rooted = Quantity<
    std::ratio_divide<typename Q::mass, quotient>,
...
// normally kg*m^2, but also J*s^2 / rad^2... rad is normally dimensionless, not here
...
NEW_UNIT(
    ...
    volts_per_meter_per_second_squared,
    VpMps2,
    1,
    1,
    -1,
...
NEW_UNIT_LITERAL(
    LinearAccelerationFeedforward, volts_per_inch_per_second_squared, VpInps2, V / inps2
)
...
NEW_UNIT(
    LinearVelocityDerivativeGain, volt_seconds_squared_per_meter, Vs2pM, 1, 1, -1, -1, 0, 0, 0, 0
)
...
NEW_UNIT(
    AngularVelocityFeedforward, volts_per_radian_per_second, VpRadPs, 1, 2, -2, -1, -1, 0, 0, 0
)
...
NEW_UNIT(
    ...
    volts_per_radian_per_second_squared,
    VpRadPs2,
    1,
    2,
    -1,
...
NEW_UNIT_LITERAL(
    AngularAccelerationFeedforward, volts_per_degree_per_second_squared, VpDegPs2, V / dps2
)
...
class Temperature : public TemperatureQuantity {
 public:
  ...
  explicit constexpr Temperature(double kelvins) : TemperatureQuantity(kelvins) {}
...
constexpr auto ceil(const T& lhs, const U& rhs) {
  ...
  auto qrhs = to_quantity(rhs);
  if consteval {
    return decltype(qlhs)(cevalm::ceil(qlhs.internal() / qrhs.internal()) * qrhs.internal());
  } else {
...
constexpr auto floor(const T& lhs, const U& rhs) {
  ...
  auto qrhs = to_quantity(rhs);
  if consteval {
    return decltype(qlhs)(cevalm::floor(qlhs.internal() / qrhs.internal()) * qrhs.internal());
  } else {
...
constexpr auto trunc(const T& lhs, const U& rhs) {
  ...
  auto qrhs = to_quantity(rhs);
  if consteval {
    return decltype(qlhs)(cevalm::trunc(qlhs.internal() / qrhs.internal()) * qrhs.internal());
  } else {
...
constexpr auto round(const T& lhs, const U& rhs) {
  ...
  auto qrhs = to_quantity(rhs);
  if consteval {
    return decltype(qlhs)(cevalm::round(qlhs.internal() / qrhs.internal()) * qrhs.internal());
  } else {
...
template <typename Q>
concept KinematicQuantity = isQuantity<Q> && std::ratio_equal_v<typename Q::mass, std::ratio<0>> &&
...
constexpr Angle wrap_signed(Angle angle) { return wrap_positive(angle + rev / 2.0) - rev / 2.0; }

constexpr Angle shortest_difference(Angle start, Angle end) { return wrap_signed(end - start); }
