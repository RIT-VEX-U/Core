...
/**
 * This class represents a quantity with unit and value
 *
...
class Quantity {
 ...
 public:
   ...
   */
  constexpr Self& operator+=(Self other) { value += other.value; return *this; }
   ...
   */
  constexpr Self& operator*=(double scalar) { value *= scalar; return *this; }

  /**
   * Divides this quantity by a scalar
   */
  constexpr Self& operator/=(double divisor) { value /= divisor; return *this; }
...
// Number is a dimensionless quantity that can be implicitly converted
...
// isQuantity concept checks whether something is a quantity
template <typename Q>
...
template <isQuantity Q1, isQuantity Q2>
using Divided = Quantity<std::ratio_subtract<typename Q1::mass, typename Q2::mass>,
...
constexpr auto operator*(Q1 lhs, Q2 rhs) {
  ...
  if constexpr (std::is_same_v<Result, DimensionlessQuantity>)
...
#define NEW_METRIC_PREFIXES(Name, long_base, base)         \
  ...
  NEW_UNIT_LITERAL(Name, giga##base, G##base, base * 1E9)  \
  NEW_UNIT_LITERAL(Name, mega##base, M##base, base * 1E6)  \
  NEW_UNIT_LITERAL(Name, kilo##base, k##base, base * 1E3)  \
  NEW_UNIT_LITERAL(Name, centi##base, c##base, base / 1E2) \
  NEW_UNIT_LITERAL(Name, milli##base, m##base, base / 1E3) \
  NEW_UNIT_LITERAL(Name, micro##base, u##base, base / 1E6) \
  NEW_UNIT_LITERAL(Name, nano##base, n##base, base / 1E9)  \
  NEW_UNIT_LITERAL(Name, pico##base, p##base, base / 1E12)

NEW_UNIT(Mass, kilograms, kg, 1, 0, 0, 0, 0, 0, 0, 0)
NEW_UNIT_LITERAL(Mass, grams, g, kg / 1000)
NEW_UNIT_LITERAL(Mass, ounces, oz, g * 28.3495)
...
NEW_UNIT_LITERAL(Area, square_yards, yd2, m2 * 0.836127)
NEW_UNIT_LITERAL(Area, square_feet, ft2, yd2 / 9)
NEW_UNIT_LITERAL(Area, square_inches, in2, ft2 / 144)
NEW_UNIT_LITERAL(Area, acres, acre, yd2 * 4840)

NEW_UNIT(Volume, cubic_meters, m3, 0, 3, 0, 0, 0, 0, 0, 0)
NEW_UNIT_LITERAL(Volume, cubic_feet, ft3, m3 * 0.0283168)
NEW_UNIT_LITERAL(Volume, cubic_inches, in3, ft3 * 0.000578704)
NEW_UNIT_LITERAL(Volume, liters, liter, m3 * 0.001)
NEW_UNIT_LITERAL(Volume, gallons, gal, liter * 3.78541)
...
NEW_UNIT(AngularAcceleration, radians_per_second_squared, radps2, 0, 0, -2, 0, 1, 0, 0, 0)
NEW_UNIT_LITERAL(AngularAcceleration, degrees_per_second_squared, dps2, deg / s / s)
NEW_UNIT_LITERAL(AngularAcceleration, revolutions_per_second_squared, revps2, rev / s / s)
...
NEW_UNIT_LITERAL(AngularJerk, revolutions_per_second_cubed, revps3, rev / (s * s * s))
...
NEW_UNIT(LinearVelocityFeedforward, volts_per_meter_per_second, VpMps, 1, 1, -2, -1, 0, 0, 0, 0)
NEW_UNIT_LITERAL(LinearVelocityFeedforward, volts_per_inch_per_second, VpInps, V / inps)
...
NEW_UNIT(LinearIntegralGain, volts_per_meter_second, VpMS, 1, 1, -4, -1, 0, 0, 0, 0)
NEW_UNIT_LITERAL(LinearIntegralGain, volts_per_inch_second, VpInS, V / (in * s))

NEW_UNIT(LinearAccelerationFeedforward, volts_per_meter_per_second_squared, VpMps2, 1, 1, -1, -1, 0, 0, 0, 0)
NEW_UNIT_LITERAL(LinearAccelerationFeedforward, volts_per_inch_per_second_squared, VpInps2, V / inps2)

NEW_UNIT(LinearDerivativeGain, volt_seconds_per_meter, VspM, 1, 1, -2, -1, 0, 0, 0, 0)
NEW_UNIT_LITERAL(LinearDerivativeGain, volt_seconds_per_inch, VspIn, V / inps)

// These are the same dimension, and we don't have special names, so "using" is cleanest
using LinearVelocityProportionalGain = LinearDerivativeGain;

NEW_UNIT(LinearVelocityDerivativeGain, volt_seconds_squared_per_meter, Vs2pM, 1, 1, -1, -1, 0, 0, 0, 0)
NEW_UNIT_LITERAL(LinearVelocityDerivativeGain, volt_seconds_squared_per_inch, Vs2pIn, V / inps2)

using LinearVelocityIntegralGain = LinearProportionalGain;

NEW_UNIT(AngularVelocityFeedforward, volts_per_radian_per_second, VpRadPs, 1, 2, -2, -1, -1, 0, 0, 0)
NEW_UNIT_LITERAL(AngularVelocityFeedforward, volts_per_degree_per_second, VpDegPs, V / dps)

NEW_UNIT(AngularProportionalGain, volts_per_radian, VpRad, 1, 2, -3, -1, -1, 0, 0, 0)
NEW_UNIT_LITERAL(AngularProportionalGain, volts_per_degree, VpDeg, V / deg)

NEW_UNIT(AngularIntegralGain, volts_per_radian_second, VpRadS, 1, 2, -4, -1, -1, 0, 0, 0)
NEW_UNIT_LITERAL(AngularIntegralGain, volts_per_degree_second, VpDegS, V / (deg * s))

NEW_UNIT(AngularAccelerationFeedforward, volts_per_radian_per_second_squared, VpRadPs2, 1, 2, -1, -1, -1, 0, 0, 0)
NEW_UNIT_LITERAL(AngularAccelerationFeedforward, volts_per_degree_per_second_squared, VpDegPs2, V / dps2)

NEW_UNIT(AngularDerivativeGain, volt_seconds_per_radian, VspRad, 1, 2, -2, -1, -1, 0, 0, 0)
NEW_UNIT_LITERAL(AngularDerivativeGain, volt_seconds_per_degree, VspDeg, V / dps)

using AngularVelocityProportionalGain = AngularDerivativeGain;

NEW_UNIT(AngularVelocityDerivativeGain, volt_seconds_squared_per_radian, Vs2pRad, 1, 2, -1, -1, -1, 0, 0, 0)
NEW_UNIT_LITERAL(AngularVelocityDerivativeGain, volt_seconds_squared_per_degree, Vs2pDeg, V / dps2)
...
template <typename T>
constexpr auto to_quantity(T value) -> std::conditional_t<std::is_arithmetic_v<T>, Number, T> {
...
constexpr auto pow(const T& lhs) {
  ...
  if consteval {
    return S(gcem::pow(q.internal(), R));
  } else {
...
constexpr auto root(const T& lhs) {
  ...
  if consteval {
    return S(gcem::pow(q.internal(), 1.0 / R));
  } else {
...
constexpr auto hypot(const T& lhs, const U& rhs) {
  ...
  auto qrhs = to_quantity(rhs);
  if consteval {
    return decltype(qlhs)(gcem::hypot(qlhs.internal(), qrhs.internal()));
  } else {
...
constexpr auto mod(const T& lhs, const U& rhs) {
  ...
  auto qrhs = to_quantity(rhs);
  if consteval {
    return decltype(qlhs)(gcem::fmod(qlhs.internal(), qrhs.internal()));
  } else {
...
constexpr auto copysign(const T& lhs, const U& rhs) {
  ...
  auto qrhs = to_quantity(rhs);
  if consteval {
    return decltype(qlhs)(gcem::copysign(qlhs.internal(), qrhs.internal()));
  } else {
...
constexpr auto ceil(const T& lhs, const U& rhs) {
  ...
  auto qrhs = to_quantity(rhs);
  if consteval {
    return decltype(qlhs)(gcem::ceil(qlhs.internal() / qrhs.internal()) * qrhs.internal());
  } else {
...
constexpr auto floor(const T& lhs, const U& rhs) {
  ...
  auto qrhs = to_quantity(rhs);
  if consteval {
    return decltype(qlhs)(gcem::floor(qlhs.internal() / qrhs.internal()) * qrhs.internal());
  } else {
...
constexpr auto trunc(const T& lhs, const U& rhs) {
  ...
  auto qrhs = to_quantity(rhs);
  if consteval {
    return decltype(qlhs)(gcem::trunc(qlhs.internal() / qrhs.internal()) * qrhs.internal());
  } else {
...
constexpr auto round(const T& lhs, const U& rhs) {
  ...
  auto qrhs = to_quantity(rhs);
  if consteval {
    return decltype(qlhs)(gcem::round(qlhs.internal() / qrhs.internal()) * qrhs.internal());
  } else {
    return decltype(qlhs)(std::round(qlhs.internal() / qrhs.internal()) * qrhs.internal());
  }
}

// Helper to go from angular to linear, like from encoder rotation to wheel distance travel
template <isQuantity Q>
requires std::ratio_equal_v<typename Q::angle, std::ratio<1>> && std::ratio_equal_v<typename Q::length, std::ratio<0>>
constexpr auto to_linear(Q angular_distance, Length diameter) {
    return angular_distance * (diameter / 2.0) / rad;
}

// Helper to go from linear to angular, like from wheel distance travel to wheel rotation
template <isQuantity Q>
