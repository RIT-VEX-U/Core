...
class Serializer {
   ...
   public:
    /// @brief Save and close upon destruction (bc of vex, this doesnt always get called when the
    /// program ends. To be sure, call save_to_disk)
    ~Serializer() {
        save_to_disk();
        printf("Saving %s\n", filename.c_str());
        fflush(stdout);
    ...
    /// @param flush_always If true, after every write flush to a file. If false, you are
    /// responsible for calling save_to_disk
    explicit Serializer(const std::string& filename, bool flush_always = true)
        : flush_always(flush_always),
          filename(filename),
          ints({}),
          bools({}),
          doubles({}),
    ...
    /// @brief sets an integer by the name of name to i. If flush_always == true, this will save to
    /// the sd card
    /// @param name name of integer
    /// @param i value of integer
    void set_int(const std::string& name, int i);

    /// @brief sets a bool by the name of name to b. If flush_always == true, this will save to the
    /// sd card
    /// @param name name of bool
    /// @param b value of bool
    void set_bool(const std::string& name, bool b);

    /// @brief sets a double by the name of name to d. If flush_always == true, this will save to
    /// the sd card
    /// @param name name of double
    /// @param d value of double
    void set_double(const std::string& name, double d);

    /// @brief sets a string by the name of name to s. If flush_always == true, this will save to
    /// the sd card
