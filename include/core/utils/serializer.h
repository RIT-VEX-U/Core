...
class Serializer {
 ...
 public:
  /// @brief Save and close upon destruction (bc of vex, this doesnt always get called when the program ends. To be
  /// sure, call save_to_disk)
  ~Serializer() {
    save_to_disk();
    printf("Saving %s\n", filename.c_str());
    fflush(stdout);
  ...
  /// @param flush_always If true, after every write flush to a file. If false, you are responsible for calling
  /// save_to_disk
