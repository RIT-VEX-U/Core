...
class RegistryController {
   ...
   private:
    ...
    CallbackFn on_broadcast = [&](VDP::Channel chan) {
        ...
        printf("VDB-Controller: No Broadcast Callback installed: Received broadcast "
               "for channel id "
               "%d:\n%s\n",
               int(chan.getID()),
               schema_str.c_str());
     ...
     * essentially moves what they give us -> what we have and then updates the parts by calling
     * response
     */
