   ...
   public:
    ...
    void take_packet(const Packet& pac) {
        ...
        if (status == VDP::PacketValidity::BadChecksum) {
            ...
            num_small++;
            VDPWarnf(
                    "Listener: Packet too small to be valid (%d bytes). Skipping", (int)pac.size()
            );
            return;
   ...
   private:
    ...
    CallbackFn on_data = [](VDP::Channel chan) {
        printf("VDB Listener: No Data Callback installed: got data for channel "
               "%d:\n%s\n",
               int(chan.id),
               chan.data->pretty_print_data().c_str());
    };
    CallbackFn on_rec = [](VDP::Channel chan) {
        printf("VDB Listener: No Data Callback installed: Received data for channel "
               "%d:\n%s\n",
               int(chan.id),
               chan.data->pretty_print_data().c_str());
    };
