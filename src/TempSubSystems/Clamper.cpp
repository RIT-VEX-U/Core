...
void ClamperSys::start_auto_clamping(){
    AutoClamping = true;
};
void ClamperSys::stop_auto_clamping(){
    AutoClamping = false;
};

void ClamperSys::clamp(){
    clamper_state = ClamperState::CLAMPED;
};
void ClamperSys::unclamp(){
    clamper_state = ClamperState::UNCLAMPED;
};

// returns true if the piston is clamped down
bool ClamperSys::is_clamped(){
    return goal_grabber_sol.value();
};
