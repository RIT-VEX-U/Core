...
TimestampedRecord::TimestampedRecord(std::string name, Part* data)
    : Record(name),
      timestamp(new Float("timestamp(sec)", []() { return (float)vexSystemTimeGet() / 1000; })),
      data(data) {
...
void OdometryDataRecord::fetch() {
    X->set_value((float)odom.get_position().x().to(units::in));
    Y->set_value((float)odom.get_position().y().to(units::in));
    ROT->set_value((float)odom.get_position().rotation_.degrees());
}
/**
...
void OdometryControlRecord::response() {
    odom.set_position(
            Pose2d(units::Length(X->get_value(), units::in),
                   units::Length(X->get_value(), units::in), ROT->get_value())
    );
}
