...
/**
 * Checks if the string has the value N/A, if it does skip it otherwise replace our data with the
 * new string
 */
...
/**
 * checks if the value if the lowest possible number it can be, if it is skip it otherwise replace
 * our datw with the new number
 */
void ResponsePacketVisitor::VisitFloat(VDP::Float* float_part) {
    ...
    if (from_float->get_value() != std::numeric_limits<float>().min()) {
        printf("found float %s, changing %f to %f\n",
               ...
               float_part->get_value(),
               from_float->get_value());
