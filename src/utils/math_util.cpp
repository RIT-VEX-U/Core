...
*/
double covariance(
        std::vector<std::pair<double, double>> const& points, double meanx, double meany
) {
    double covar = 0.0;
...
std::pair<double, double> calculate_linear_regression(
    ...
    // Purely for convenience and the ability to reuse mean() and variance() - can be easily
    // rewritten to avoid allocating these if the code is repeatedly called
