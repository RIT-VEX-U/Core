...
Trajectory TrajectoryParameterizer::jerk_limit_trajectory(
  ...
  units::Time t_window = max_acceleration / max_jerk;
  if (t_window < 10_ms) {
    return base;
  }

  units::Time dt = 10_ms;
  units::Time old_time = base.total_time();
  units::Time new_time = old_time + t_window;

  int num_samples = static_cast<int>(std::ceil((new_time / dt).internal())) + 1;
  std::vector<Trajectory::State> new_states;
  new_states.reserve(num_samples);

  const auto& base_states = base.states();
  std::vector<units::Length> base_s(base_states.size());
  base_s[0] = 0_in;
  for (size_t i = 1; i < base_states.size(); ++i) {
    base_s[i] = base_s[i - 1] + units::in * (
        base_states[i].pose.translation().distance(base_states[i - 1].pose.translation()));
  }
  units::Length total_s = base_s.back();

  units::Velocity current_v = 0_inps;
  units::Length current_s = 0_in;

  for (int i = 0; i < num_samples; ++i) {
    units::Time t = i * dt;
    units::Time t_start = t - t_window;
    
    const int sub_samples = 50;
    units::Time sub_dt = t_window / sub_samples;
    units::Velocity v_sum = 0_inps;
    
    for (int j = 0; j <= sub_samples; ++j) {
      units::Time sample_t = t_start + (j * sub_dt);
    ...
    units::Velocity v_avg = v_sum / (sub_samples + 1);
    ...
    if (upper == base_s.end()) {
      ...
      units::Length s0 = base_s[idx - 1];
      units::Length s1 = base_s[idx];
      units::Time t0 = base_states[idx - 1].t;
      units::Time t1 = base_states[idx].t;
      
      double alpha = (s1 == s0) ? 0.0 : ((current_s - s0) / (s1 - s0)).internal();
      t_base = t0 + (t1 - t0) * alpha;
    }
    
    Trajectory::State mapped_state = base.sample(t_base);
    
    units::Acceleration a = 0_inps2;
    if (i > 0) {
