...
Logger::Logger(const std::string &filename) : filename(filename) { sd.savefile(filename.c_str(), NULL, 0); }

void Logger::Log(const std::string &s) { sd.appendfile(filename.c_str(), (unsigned char *)s.c_str(), s.size()); }
