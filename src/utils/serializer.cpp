...
template <typename T> std::vector<char> to_bytes(T value) {
    // copy bytes of data into vector
    std::vector<char> value_bytes(sizeof(T));
    std::copy(
      static_cast<const char *>(static_cast<const void *>(&value)),
      static_cast<const char *>(static_cast<const void *>(&value)) + sizeof(value), value_bytes.begin()
    );
    return value_bytes;
}
...
template <typename T> T from_bytes(std::vector<char>::const_iterator &position) {
    T value;
...
template <typename value_type>
static std::vector<char>::const_iterator
read_data(std::vector<char>::const_iterator begin, std::map<std::string, value_type> &map) {
