...
PIDFF::PIDFF(PID& pid, FeedForward& feedforward) : pid(pid), feedforward(feedforward) {
  out = 0;
  lower_lim = 0;
  upper_lim = 0;
}
...
double PIDFF::update(double val) {
  ...
  double ff_out = feedforward.kG + (feedforward.kS * sign(pid_out));
  out = pid_out + ff_out;
...
double PIDFF::update(double val, double vel_setpt, double a_setpt) {
  double pid_out = pid.update(val);
  double ff_out = feedforward.calculate(vel_setpt, a_setpt);
  out = pid_out + ff_out;
  if (lower_lim != upper_lim) {
    out = clamp(out, lower_lim, upper_lim);
  }
