  ...
  for (int i = 0; i < points.size() - 1; i++) {
    for (int j = i + 2; j < points.size() - 1; j++) {
      ...
      for (double t1 = 0; t1 <= 1; t1 += radius / segment_i_dist) {
        ...
        p1 = Translation2d((points[i].x() + t1 * (points[i + 1].x() - points[i].x())),
                           (points[i].y() + t1 * (points[i + 1].y() - points[i].y())));

        for (double t2 = 0; t2 <= 1; t2 += radius / segment_j_dist) {
          Translation2d p2(0, 0);
          p2 = Translation2d((points[j].x() + t2 * (points[j + 1].x() - points[j].x())),
 ...
 */
std::vector<Translation2d> PurePursuit::line_circle_intersections(Translation2d center, double r, Translation2d point1,
 ...
 */
[[maybe_unused]] Translation2d PurePursuit::get_lookahead(const std::vector<Translation2d>& path, Pose2d robot_loc,
                                                          double radius) {
...
*/
[[maybe_unused]] std::vector<Translation2d> PurePursuit::inject_path(const std::vector<Translation2d>& path,
                                                                     double spacing) {
 ...
 */
[[maybe_unused]] std::vector<Translation2d> PurePursuit::smooth_path(const std::vector<Translation2d>& path,
