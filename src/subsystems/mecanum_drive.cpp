...
bool MecanumDrive::auto_drive(double inches, double direction, double speed, bool gyro_correction) {
  ...
  // Some wheels are NOT being averaged at any given time since the general mecanum algorithm makes them go slower than
  // our robot speed somewhat of a nasty hack, but wheel slippage should make up for it, and multivariable calc is hard.
  ...
  if (enable_wheel) {
    ...
    dist_avg = sqrt(pow(lateral_wheel->position(rotationUnits::rev) * config->lateral_wheel_diam * PI, 2) +
