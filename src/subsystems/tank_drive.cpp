...
AutoCommand* TankDrive::DriveToPointCmd(double x, double y, vex::directionType dir, double max_speed,
                                        double end_speed) {
...
AutoCommand* TankDrive::TurnToPointCmd(Translation2d point, vex::directionType dir, double max_speed,
                                       double end_speed) {
...
bool TankDrive::pure_pursuit(PurePursuit::Path path, directionType dir, Feedback& feedback, double max_speed,
  ...
  if (dir != directionType::rev) {
    ...
    angle_diff = OdometryBase::smallest_angle(robot_pose.rotation().degrees() + 180,
                                              rad2deg(atan2(localized.y(), localized.x())));
  }
