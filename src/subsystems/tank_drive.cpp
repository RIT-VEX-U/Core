...
TankDrive::TankDrive(
    ...
    : left_motors(left_motors), right_motors(right_motors), odometry(odom), config(config) {
...
void TankDrive::drive_tank(double left, double right, int power, BrakeType bt) {
  ...
  bool should_brake =
      (bt != BrakeType::None) && fabs(left) < brake_threshold && fabs(right) < brake_threshold;
  ...
  static PID zero_vel_pid(0.005, 0, 0.0005);
...
bool TankDrive::drive_forward(
  ...
  printf(
      "tank_drive.cpp: Cannot run drive_forward without a feedback "
      "controller!\n"
  );
  fflush(stdout);
  return true;
...
bool TankDrive::turn_degrees(double degrees, double max_speed, double end_speed) {
  ...
  printf(
      "tank_drive.cpp: Cannot run turn_degrees without a feedback "
      "controller!\n"
  );
  fflush(stdout);
  return true;
...
bool TankDrive::drive_to_point(
  ...
  if (!func_initialized) {
    ...
    if (config.correction_pid != nullptr) {
      // Reset the control loops
      config.correction_pid->init(0, 0);

      config.correction_pid->set_limits(-1, 1);
    }
  ...
  if (config.correction_pid != nullptr) {
    config.correction_pid->update(delta_heading);
  }
  ...
  if ((is_pure_pursuit || fabs(dist_left) > config.drive_correction_cutoff) &&
      config.correction_pid != nullptr) {
    correction = config.correction_pid->get();
  }
...
bool TankDrive::drive_to_point(
  ...
  printf(
      "tank_drive.cpp: Cannot run drive_to_point without a feedback "
      "controller!\n"
  );
  fflush(stdout);
  return true;
...
bool TankDrive::turn_to_heading(
  ...
  if (!func_initialized) {
    double initial_delta =
        OdometryBase::smallest_angle(odometry->get_position().rotation().degrees(), heading_deg);
...
bool TankDrive::turn_to_heading(double heading_deg, double max_speed, double end_speed) {
  ...
  printf(
      "tank_drive.cpp: Cannot run turn_to_heading without a feedback "
      "controller!\n"
  );
  fflush(stdout);
  return true;
...
bool TankDrive::pure_pursuit(
  ...
  if (dir != vex::directionType::rev) {
    angle_diff = OdometryBase::smallest_angle(
        robot_pose.rotation().degrees(), rad2deg(atan2(localized.y(), localized.x()))
    );
  } else {
    angle_diff = OdometryBase::smallest_angle(
  ...
  if (!(is_last_point &&
      ...
      config.correction_pid != nullptr) {
    config.correction_pid->update(angle_diff);
    correction = config.correction_pid->get();
